...
import { makeUtils } from "./utils/jscodeshift-utils";

/**
 * @param {string} location
 */
export async function scopedCssVite(location) {
      ...
      j(path)
        ...
        .forEach((path) => {
          ...
          const newPlugin = j.callExpression(
            j.memberExpression(
              j.identifier("scopedCssUnplugin"),
              j.identifier("rollup")
            ),
            []
          );
 ...
 * @param {string} location
 */
export async function scopedCssEmberCli(location) {
  await packageJson.addDevDependencies(
    {
      "ember-scoped-css": "0.21.4",
  ...
  await js.transform(emberCliBuild, async ({ root, j }) => {
    root
      .find(j.CallExpression, { callee: { property: { name: "compatBuild" } } })
      .forEach((path) => {
